        ...
        <table class="w-full text-left border-collapse">
            ...
            <tbody class="divide-y divide-slate-50">
                ...
                <tr class="group hover:bg-slate-50/50 transition-colors">
                    ...
                    <td class="py-4 px-4">
                        <span class="text-xs font-black text-primary px-2.5 py-1 bg-blue-50 rounded-lg">KOL{{ sprintf('%03d', $collector->id) }}</span>
                    </td>
                    <td class="py-4 px-4 text-sm font-black text-slate-900">{{ $collector->nama }}</td>
                    <td class="py-4 px-4 text-xs font-bold text-slate-500 uppercase tracking-wider">{{ $collector->wilayah }}</td>
                    <td class="py-4 px-4 text-center">
                        <span class="text-xs font-bold text-slate-600">{{ $collector->no_hp }}</span>
                    </td>
                    <td class="py-4 px-4 text-center">
                    ...
                    <td class="py-4 px-8 text-center">
                        <div class="flex justify-center gap-2">
                            ...
                            <form action="{{ route('collectors.destroy', $collector->id) }}" method="POST" class="inline" onsubmit="return confirm('Yakin hapus?')">
                                @csrf @method('DELETE')
...
<div id="addModal" class="hidden fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
    <div class="bg-white w-full max-w-md rounded-[2.5rem] shadow-2xl p-8 animate-fade-in-up">
        ...
        <form action="{{ route('collectors.store') }}" method="POST" class="space-y-6">
            @csrf
            <div>

...
<div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
    ...
    <div class="flex flex-wrap gap-3">
        ...
        <a href="{{ route('pbbs.export') }}" class="flex items-center gap-2 bg-rose-500 hover:bg-rose-600 text-white px-5 py-2.5 rounded-xl font-bold text-sm transition-all shadow-lg shadow-rose-500/20 active:scale-95">
...
<div class="bg-white p-6 rounded-[2rem] shadow-premium border border-slate-50 mb-8">
    <form action="{{ route('pbbs.index') }}" method="GET" class="flex flex-col lg:flex-row gap-4">
        <div class="flex flex-col md:flex-row gap-4 flex-1">
            ...
            <div class="relative flex-1">
                ...
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari NOP atau Nama Wajib Pajak..." 
        ...
        <div class="flex gap-2">
            ...
            <a href="{{ route('pbbs.index') }}" class="bg-slate-100 text-slate-600 px-6 py-2.5 rounded-xl font-bold text-sm hover:bg-slate-200 transition-all active:scale-95">
                Reset
            </a>
            @endif
        </div>
...
<div class="bg-white rounded-[2.5rem] shadow-premium border border-slate-50 overflow-hidden">
    <div class="overflow-x-auto">
        <table class="w-full text-left border-collapse">
            ...
            <tbody class="divide-y divide-slate-50">
                @forelse($pbbs as $index => $pbb)
                <tr class="group hover:bg-slate-50/50 transition-colors">
                    <td class="py-4 px-8 text-xs font-bold text-slate-400">{{ $pbb->no_urut }}</td>
                    <td class="py-4 px-4 text-xs font-bold text-slate-600">{{ $pbb->blok }}</td>
                    <td class="py-4 px-4 text-xs font-bold text-slate-600">{{ $pbb->urut }}</td>
                    <td class="py-4 px-4 text-xs font-bold text-slate-600">{{ $pbb->nop_gabung }}</td>
                    <td class="py-4 px-4 text-xs font-black text-primary">{{ $pbb->nop }}</td>
                    <td class="py-4 px-4 text-sm font-black text-slate-900">{{ $pbb->nama_wp }}</td>
                    <td class="py-4 px-4 text-sm text-slate-500">{{ $pbb->nama_wp_lainnya ?? '-' }}</td>
                    <td class="py-4 px-4 text-sm font-bold text-slate-900">Rp {{ number_format($pbb->ketetapan_pbb, 0, ',', '.') }}</td>
                    <td class="py-4 px-4 text-sm text-slate-600">{{ $pbb->nama_kolektor ?? '-' }}</td>
                    <td class="py-4 px-4 text-sm text-slate-600">{{ $pbb->luas }}</td>
                    <td class="py-4 px-4 text-xs text-slate-500">{{ $pbb->alamat_wajib_pajak }}</td>
                    <td class="py-4 px-4 text-sm font-bold text-rose-500 text-right">Rp {{ number_format($pbb->hutang_pbb, 0, ',', '.') }}</td>
                    <td class="py-4 px-4 text-xs text-center text-slate-500">{{ $pbb->tgl_bayar ?? '-' }}</td>
                    <td class="py-4 px-4 text-sm font-bold text-emerald-500 text-right">Rp {{ number_format($pbb->jumlah_bayar, 0, ',', '.') }}</td>
                    <td class="py-4 px-4 text-center">
                        ...
                        @endphp
                        <span class="inline-flex items-center gap-1.5 py-1 px-3 rounded-full text-[10px] font-black {{ $badgeClass }} uppercase tracking-wider">
                            <i class="fas {{ $icon }} text-[8px]"></i> {{ $pbb->status }}
                        </span>
                    </td>
                    <td class="py-4 px-4 text-xs text-slate-400">{{ $pbb->column1 ?? '-' }}</td>
                    @if(in_array(auth()->user()->role, ['admin', 'petugas']))
                    <td class="py-4 px-8 text-center">
                        <div class="flex justify-center gap-2">
                            <button onclick="openEditModal({{ $pbb->id }})" class="w-8 h-8 flex items-center justify-center rounded-lg bg-blue-50 text-primary hover:bg-primary hover:text-white transition-all">
    ...
    <div class="p-8 bg-slate-50/50 border-t border-slate-100 flex justify-between items-center">
        <p class="text-xs font-bold text-slate-400 uppercase tracking-widest">Menampilkan {{ $pbbs->firstItem() ?? 0 }} - {{ $pbbs->lastItem() ?? 0 }} dari {{ $pbbs->total() }} data</p>
        <div class="flex gap-2">
            {{ $pbbs->links('pagination::tailwind') }}
        </div>
    </div>
</div>
...
<div id="importModal" class="hidden fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
    <div class="bg-white w-full max-w-md rounded-[2.5rem] shadow-2xl p-8 animate-fade-in-up">
        ...
        <form action="{{ route('pbbs.import') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf

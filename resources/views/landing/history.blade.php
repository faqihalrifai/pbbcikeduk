    ...
    <main class="flex-grow container mx-auto px-6 py-12">
        <div class="max-w-6xl mx-auto">
            ...
            <div class="bg-white rounded-[2.5rem] p-8 shadow-premium border border-slate-50 mb-10">
                <form action="{{ route('landing.history') }}" method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-6 items-end">
                    <div class="md:col-span-1">
                        ...
                        <div class="relative">
                            ...
                            <input type="text" name="search" value="{{ $search }}" placeholder="Nama atau NOP..." 
                    ...
                    <div class="flex gap-2">
                        ...
                        <a href="{{ route('landing.history') }}" class="bg-slate-100 text-slate-500 p-3 rounded-xl hover:bg-slate-200 transition-all">
                    ...
                    <table class="w-full text-left">
                        ...
                        <tbody class="divide-y divide-slate-100">
                            ...
                            <tr class="group hover:bg-slate-50/50 transition-all duration-300">
                                <td class="py-6 px-8">
                                    <p class="text-sm font-black text-slate-900">{{ \Carbon\Carbon::parse($pay->tgl_bayar)->format('d/m/Y') }}</p>
                                </td>
                                <td class="py-6 px-6">
                                    <p class="font-black text-slate-900 text-sm mb-0.5">{{ $pay->nama_wp }}</p>
                                    <p class="text-[10px] font-bold text-slate-400 uppercase tracking-wider">{{ $pay->nop }}</p>
                                </td>
                                <td class="py-6 px-6">
                                    <p class="text-xs font-medium text-slate-500 line-clamp-1">{{ $pay->alamat_wajib_pajak }}</p>
                                </td>
                                <td class="py-6 px-6 text-right">
                                    <span class="text-sm font-black text-slate-900">Rp {{ number_format($pay->jumlah_bayar, 0, ',', '.') }}</span>
                                </td>

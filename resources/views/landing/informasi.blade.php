    ...
    <main class="flex-grow container mx-auto px-6 py-12">
        <div class="max-w-6xl mx-auto">
            ...
            <div class="flex flex-col md:flex-row justify-between items-center gap-6 mb-12">
                ...
                <form action="{{ route('landing.informasi') }}" method="GET" class="w-full md:w-96 relative">
                    <i class="fas fa-search absolute left-5 top-1/2 -translate-y-1/2 text-slate-400"></i>
                    <input type="text" name="search" value="{{ $search }}" placeholder="Cari Nama atau NOP..." 
            ...
            <div class="bg-white rounded-[2.5rem] shadow-premium border border-slate-50 overflow-hidden">
                    ...
                    <table class="w-full text-left">
                        ...
                        <tbody class="divide-y divide-slate-100">
                            ...
                            <tr class="group hover:bg-slate-50/50 transition-all duration-300">
                                <td class="py-6 px-8">
                                    <p class="font-black text-slate-900 text-sm mb-0.5">{{ $item->nama_wp }}</p>
                                    <p class="text-[11px] font-bold text-slate-400 uppercase tracking-wider">{{ $item->nop }}</p>
                                </td>
                                <td class="py-6 px-6">
                                    <p class="text-xs font-medium text-slate-500 line-clamp-2 max-w-[200px]">{{ $item->alamat_wajib_pajak }}</p>
                                </td>
                                <td class="py-6 px-6 text-right">
                                    <span class="text-sm font-black text-primary">Rp {{ number_format($item->ketetapan_pbb, 0, ',', '.') }}</span>
                                </td>
                                ...
                                <td class="py-6 px-8 text-center">
                                    @if($item->tgl_bayar)
                                    <p class="text-sm font-bold text-slate-900">{{ \Carbon\Carbon::parse($item->tgl_bayar)->format('d/m/Y') }}</p>
                                    @else
                ...
                <div class="bg-slate-50/50 px-8 py-6 border-t border-slate-100">
                    {{ $pbb->appends(['search' => $search])->links() }}
                </div>
                @endif
            </div>

...
<body class="bg-background text-slate-700 antialiased">
    ...
    <nav class="fixed top-0 w-full z-50 glass-nav border-b border-slate-100">
        <div class="container mx-auto px-6 py-4 flex justify-between items-center">
            ...
            <div class="hidden md:flex items-center gap-8 text-sm font-semibold text-slate-600">
                ...
                <a href="{{ route('landing.balik_nama') }}" class="hover:text-primary transition-colors">Balik Nama</a>
        ...
        <div class="container mx-auto px-6 flex flex-col md:flex-row items-center">
            <div class="md:w-1/2 mb-12 md:mb-0">
                ...
                <div class="flex flex-wrap gap-4">
                    <a href="{{ route('login') }}" class="bg-slate-900 text-white px-8 py-3.5 rounded-xl font-bold shadow-xl hover:bg-slate-800 transition-all active:scale-95">Login Admin</a>
                    <a href="{{ route('kolektor.dashboard') }}" class="bg-white text-slate-900 border border-slate-200 px-8 py-3.5 rounded-xl font-bold shadow-lg hover:bg-slate-50 transition-all active:scale-95">Login Kolektor</a>
            ...
            <div class="md:w-1/2 relative">
                <div class="relative z-10 animate-float">
                    ...
                    <div class="w-full h-80 bg-gradient-to-br from-blue-100 to-blue-50 rounded-3xl flex items-center justify-center border border-white shadow-2xl overflow-hidden">
                        ...
                        <img src="{{ asset('rumah.jpeg') }}" alt="House Illustration" class="absolute inset-0 w-full h-full object-cover mix-blend-multiply opacity-80">
                    </div>
                </div>
                <!-- Decorative Elements -->
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
            <div class="bg-white p-6 rounded-2xl shadow-premium border border-slate-50 flex items-center gap-4">
                ...
                <div>
                    ...
                    <h3 class="text-2xl font-black text-slate-900">{{ number_format($stats['total_wp'], 0, ',', '.') }}</h3>
                </div>
            </div>
            <div class="bg-white p-6 rounded-2xl shadow-premium border border-slate-50 flex items-center gap-4">
                ...
                <div>
                    ...
                    <h3 class="text-2xl font-black text-slate-900">Rp {{ number_format($stats['total_bayar'] / 1000000, 1, ',', '.') }}M</h3>
                </div>
            </div>
            <div class="bg-white p-6 rounded-2xl shadow-premium border border-slate-50 flex items-center gap-4">
                ...
                <div>
                    ...
                    <h3 class="text-2xl font-black text-slate-900">Rp {{ number_format($stats['total_tunggakan'] / 1000000, 1, ',', '.') }}M</h3>
                </div>
            </div>
            <div class="bg-white p-6 rounded-2xl shadow-premium border border-slate-50 flex items-center gap-4">
                ...
                <div>
                    ...
                    <h3 class="text-2xl font-black text-slate-900">{{ $stats['total_kolektor'] }}</h3>
                </div>
            </div>
        </div>
    </section>
                            ...
                            <tr class="group hover:bg-white transition-all duration-300">
                                <td class="py-5 px-4">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            <p class="text-sm font-black text-slate-900">{{ $pay->tunggakan->wajibPajak->nama }}</p>
                                            <p class="text-[10px] font-bold text-slate-400 uppercase">{{ $pay->tunggakan->wajibPajak->nop }}</p>
                                        </div>
                                    </div>
                                </td>
                                <td class="py-5 px-4 text-center">
                                    <span class="text-xs font-bold text-slate-500 uppercase tracking-wider">{{ $pay->tunggakan->wajibPajak->alamat }}</span>
                                </td>
                                ...
                                <td class="py-5 px-4 text-center">
                                    <span class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{{ $pay->created_at->diffForHumans() }}</span>
                                </td>
                            </tr>
                            @empty
                            <tr>
    ...
    <section class="py-24 container mx-auto px-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-8">
            <a href="{{ route('landing.payment') }}" class="group bg-white p-8 rounded-3xl border border-slate-100 shadow-sm hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 text-left">
            ...
            <a href="{{ route('landing.sppt') }}" class="group bg-white p-8 rounded-3xl border border-slate-100 shadow-sm hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 text-left">
                <div class="w-16 h-16 bg-blue-50 text-primary rounded-2xl flex items-center justify-center mb-6 group-hover:bg-primary group-hover:text-white transition-colors">
            ...
            <a href="{{ route('landing.history') }}" class="group bg-white p-8 rounded-3xl border border-slate-100 shadow-sm hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 text-left">
                <div class="w-16 h-16 bg-blue-50 text-primary rounded-2xl flex items-center justify-center mb-6 group-hover:bg-primary group-hover:text-white transition-colors">
            ...
            <a href="{{ route('landing.complaint') }}" class="group bg-white p-8 rounded-3xl border border-slate-100 shadow-sm hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 text-left">
                <div class="w-16 h-16 bg-blue-50 text-primary rounded-2xl flex items-center justify-center mb-6 group-hover:bg-primary group-hover:text-white transition-colors">
            ...
            <a href="{{ route('landing.balik_nama') }}" class="group bg-white p-8 rounded-3xl border border-slate-100 shadow-sm hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 text-left">
                <div class="w-16 h-16 bg-blue-50 text-primary rounded-2xl flex items-center justify-center mb-6 group-hover:bg-primary group-hover:text-white transition-colors">
        ...
        <div class="container mx-auto px-6 relative z-10">
            <div class="max-w-4xl mx-auto">
                ...
                <div class="bg-white/5 backdrop-blur-xl p-8 rounded-3xl border border-white/10 shadow-2xl">
                    <form action="{{ route('landing.cek') }}" method="POST" class="flex flex-col md:flex-row gap-4">
                        @csrf
                ...
                <div class="mt-8 bg-white text-slate-900 rounded-3xl p-8 shadow-2xl animate-fade-in-up">
                    <div class="flex flex-col md:flex-row justify-between items-start md:items-center border-b border-slate-100 pb-6 mb-6">
                        <div>
                            ...
                            <p class="text-slate-500">NOP: {{ $pbb->nop }}</p>
                        </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div class="md:col-span-2 bg-slate-50 p-6 rounded-2xl flex justify-between items-center">
                            <div>
                                ...
                                <p class="text-3xl font-black text-primary">Rp {{ number_format($pbb->jumlah_tagihan, 0, ',', '.') }}</p>
                            </div>
                            @if($pbb->status != 'Lunas')
            ...
            <div class="md:w-1/2">
                <div class="relative">
                    <div class="w-full h-[400px] bg-slate-100 rounded-[3rem] overflow-hidden shadow-2xl relative z-10">
                        <img src="{{ asset('rumah.jpeg') }}" alt="Kantor Desa Cikeduk" class="w-full h-full object-cover">
                    </div>
        ...
        <div class="container mx-auto px-6 relative z-20">
            <div class="grid grid-cols-1 lg:grid-cols-4 gap-16 mb-20 border-b border-white/10 pb-16">
                ...
                <div>
                    ...
                    <ul class="space-y-4 text-sm font-bold">
                        <li><a href="{{ route('landing.informasi') }}" class="text-slate-300 hover:text-white transition-colors">Informasi PBB</a></li>
                        <li><a href="#cek-tagihan" class="text-slate-300 hover:text-white transition-colors">Cek Tagihan</a></li>
                        <li><a href="{{ route('landing.payment') }}" class="text-slate-300 hover:text-white transition-colors">Pembayaran</a></li>
                        <li><a href="{{ route('landing.complaint') }}" class="text-slate-300 hover:text-white transition-colors">Pengaduan</a></li>
                        <li><a href="{{ route('landing.balik_nama') }}" class="text-slate-300 hover:text-white transition-colors">Balik Nama PBB</a></li>
                    </ul>
                </div>
            </div>
            ...
            <div class="bg-white/5 backdrop-blur-md rounded-[2.5rem] p-10 border border-white/10 mb-16 shadow-2xl">
                    ...
                    <div class="flex flex-wrap justify-center gap-6">
                        <div class="text-center">
                            ...
                            <div class="flex -space-x-3">
                                @foreach(['AR', 'SR', 'SL', 'SS'] as $initial)
                                <div class="w-10 h-10 rounded-full border-2 border-slate-800 bg-primary flex items-center justify-center text-[10px] font-black shadow-lg" title="Anggota">{{ $initial }}</div>
                                @endforeach
                            </div>
                        </div>
                ...
                <div class="grid grid-cols-2 md:grid-cols-5 gap-4 mt-10 pt-8 border-t border-white/5">
                    @foreach(['Ainur Rizki', 'Silvana Rohimatul Jannah', 'Siti Liza Azzahrah', 'Syifa Salsabilah'] as $name)
                    <div class="flex items-center gap-2 px-4 py-2 bg-white/5 rounded-xl border border-white/5 group hover:bg-primary transition-all cursor-default">
                        ...
                        <span class="text-[10px] font-bold text-slate-300 group-hover:text-white">{{ $name }}</span>
                    </div>
                    @endforeach
                </div>
            </div>
